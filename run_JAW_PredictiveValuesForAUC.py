...
if __name__ == "__main__":
    ...
    ## python run_JAW_PredictiveValuesForAUC.py dataset muh_fun bias ntrial alpha
    ## python run_JAW_PredictiveValuesForAUC.py airfoil RF 1 20 0.1
    ...
    method_names = ['jackknife+_not_sorted', 'jackknife+_sorted', 'weights_JAW_train', 'weights_JAW_test', 'muh_vals_testpoint', 'naive', 'jackknife', 'split', 'CV+', 'split_sorted', 'weights_split_train', 'weights_split_test']
    ...
    PVs_all = pd.DataFrame(columns = col_names)
    ...
    for itrial in range(ntrial):
        ...
        ## Get Full data weights if weighted == True
        weights_full = get_w(X, X_full, dataset, bias).reshape(len(X_full)) ## Get weights for the full data
        
        PVs = compute_PVs(X,Y,X1,alpha,muh_fun, weights_full, dataset, bias)
        for method in method_names:
            if(method in ['weights_JAW_test', 'muh_vals_testpoint']):
                ...
                info = pd.DataFrame(np.tile([itrial,dataset,muh_fun_name,method,False], (n, 1)), \
                                columns = ['itrial','dataset','muh_fun','method','testpoint'])
            info_PD_method_results = pd.concat([info, PVs[method]], axis = 1, ignore_index=True)
            info_PD_method_results.reset_index(drop=True, inplace=True)
            info_PD_method_results.columns = PVs_all.columns
            PVs_all.reset_index(drop=True, inplace=True)
            PVs_all = pd.concat([PVs_all, info_PD_method_results], ignore_index=True, axis=0)
            PVs_all.reset_index(drop=True, inplace=True)


        test_point_row = \
        pd.DataFrame(np.concatenate(([itrial,dataset,muh_fun_name,'any',True],Y1.squeeze(),Y1.squeeze()))).T
        test_point_row.columns = PVs_all.columns
        test_point_row.reset_index(drop=True, inplace=True)
        PVs_all.reset_index(drop=True, inplace=True)
        PVs_all = pd.concat([PVs_all, test_point_row], ignore_index=True, axis=0)
        PVs_all.reset_index(drop=True, inplace=True)

    PVs_all.to_csv(os.getcwd().removesuffix('bash_scripts') + 'results/JAWE_error_assessment/'+ str(date.today()) + '_' + dataset + '_' + muh_fun_name + '_' + str(bias) + 'Bias_' + str(ntrial) +'Trials_PVs.csv',index=False)
